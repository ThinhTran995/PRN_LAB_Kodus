...
|-------|-----------|
| API | `PRN232.[LMS].API` |
| Services | `PRN232.[LMS].Services` |
| Repositories | `PRN232.[LMS].Repositories` |
...
services:
  ...
  api:
    ...
    environment:
      ...
      - ConnectionStrings__DefaultConnection=Host=db;Port=5432;Database=lmsdb;Username=postgres;Password=12345
    ports:
      - "8080:80"
    networks:
      - lms_network
...
{
  "ConnectionStrings": {
    "DefaultConnection": "Host=localhost;Port=5432;Database=lmsdb;Username=postgres;Password=12345"
  }
}
```
